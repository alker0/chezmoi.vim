...
| `g:chezmoi#executable_path`       | \<empty string>                                          | If set, should point to the location of the chezmoi binary, and it will be used to resolve the source directory |
| `g:chezmoi#source_dir_path`       | The value returned by `chezmoi source-path` (if path to chezmoi is set) or `$XDG_DATA_HOME/chezmoi` or `$HOME/.local/share/chezmoi` | Source Directory managed by chezmoi |
